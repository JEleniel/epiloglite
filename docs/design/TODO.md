...
- Actions
    ...
    + Add or validate CI workflow that runs: `cargo fmt -- --check`, `cargo clippy`, `cargo test` and `cargo test --features no-std` when applicable.
...
- Risks
  ...
  Immediate next steps (1-2 week sprint)
    ...
    + 2. Add a minimal CI workflow (GitHub Actions) that runs `cargo fmt -- --check`, `cargo clippy`, and `cargo test` on PRs.
