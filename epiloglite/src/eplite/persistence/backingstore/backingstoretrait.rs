...
use crate::eplite::persistence::backingstore::Page;
use epiloglite_core::JournalEntry;
...
pub trait BackingStore {
    ...
    /// Reads a page from the backing store. Page id is a canonical u128.
    fn read_page(&mut self, page_id: u128) -> Result<Page, BackingStoreError>;

    /// Writes a page to the backing store.
    fn write_page(&mut self, page: Page) -> Result<(), BackingStoreError>;

    /// Allocates a new page in the backing store and returns its page id
    fn allocate_page(&mut self) -> Result<u128, BackingStoreError>;

    /// Return the page size used by the backing store (bytes)
    fn page_size(&self) -> usize;

    /// Free a page in the backing store.
    fn free_page(&mut self, page_id: u128) -> Result<(), BackingStoreError>;
